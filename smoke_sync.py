...
from jobs_bot.models import Source, Job, JobEnrichment
...
def sha1_uid(ats_type: str, company_slug: str, job_id: str) -> str:
    s = f"{ats_type}:{company_slug}:{job_id}".encode("utf-8")
...
def main():
    ...
    with SessionLocal() as session:
        ...
        if not src:
            src = Source(ats_type=ats_type, company_slug=company_slug, api_base=api_base, is_active=1)
            session.add(src)
            session.commit()
        ...
        # sync to Notion
        n = sync_pending_jobs(session, notion, limit=10, fit_min=settings.fit_min)
        print(f"Synced {n} job(s) to Notion.")

        # print resulting notion page id
        job = session.get(Job, job_uid)
